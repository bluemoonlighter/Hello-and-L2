...
|`orderType`|`int`|下单方式|可选值参考[orderType-下单方式在新窗口打开](https://dict.thinktrader.net/innerApi/enum_constants.html#ordertype-%E4%B8%8B%E5%8D%95%E6%96%B9%E5%BC%8F)
可选按股票数量买卖或按照金额等方式买卖
一、期货不支持 1102 和 1202;
...
|`strategyName`|`string`|自定义策略名|一、用来区分 `order` 委托和`deal` 成交来自不同的策略。
...
|`quickTrade`|`int`|设定是否立即触发下单|可选值参考[quicktrade - 快速下单在新窗口打开](https://dict.thinktrader.net/innerApi/enum_constants.html#quicktrade-%E5%BF%AB%E9%80%9F%E4%B8%8B%E5%8D%95)
